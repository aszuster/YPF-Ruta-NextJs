...
import TableArticles from 'components/tables/admin/TableArticles'
// Styles
import styles from './index.module.css'
import TablePagination from 'components/tables/TablePagination'
import { allCategories } from 'services/categories'

function ArticlesAdminPage({ limit, page, category }) {
  ...
  const { data, isLoading, isError } = useQuery(
    ...
    () => filterArticles({ limit: limit, page: page, category: category })
  )
  //Submit form
  ...
  return (
    <LayoutAdmin>
      {/* Título y Botones */}
      <AdminTitle title="Artículos">
        <AdminIconButton
          action={() => router.push('/admin/articles/new_article')}
        >
          <svg
            width={20}
            height={20}
      ...
      {/* Tabla */}
      <TableArticles articles={data?.data.response} isLoading={isLoading} />
