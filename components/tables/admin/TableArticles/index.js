...
const TableArticles = ({ articles, isLoading }) => {
    ...
    promiseToast(
      (resolve) =>
        mutate(
          ...
          {
            onSuccess: () => {
              queryClient.removeQueries(['article', id], { exact: true })
  ...
  const handleSwitchState = (item) => {
    let headers = { id: item.id, state: item.state === 0 ? 1 : 0 }
    promiseToast(
      (resolve) =>
        mutateEdit([{ id: item.id }, { ...headers }], {
          onSuccess: () => {
            queryClient.invalidateQueries(['article'], item.id)
            queryClient.invalidateQueries(['articles'])
            resolve()
            router.push('/admin/articles')
          },
        }),
      'Modificando estado del articulo',
      'Estado del articulo modificado',
      'Error al modificar el estado del articulo'
    )
  }
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <AdminIconButton
                      action={() =>
                        router.push(
                          `/admin/articles/edit_article/${
                            item.id
                          }?img_path=${encodeURIComponent(item.picture)}`
                        )
                      }
                    >
                      <svg
                  ...
                  <td>
                    <AdminIconButton
                      action={() => handleDelete(item.id, item.picture)}
                    >
                      <svg
                        width={20}
                        height={20}
