...
import { deleteWebinar } from 'services/webinars'

const TableWebinars = ({ webinars, isLoading }) => {
  const { mutate } = useMutation(deleteWebinar)
    ...
    promiseToast(
      (resolve) =>
        mutate(
          ...
          {
            onSuccess: () => {
              queryClient.removeQueries(['webinar', id], { exact: true })
              queryClient.invalidateQueries(['webinars'])
              resolve()
            },
          }
        ),
      'Eliminando el webinar',
      'Webinar eliminado',
      'Error al eliminar el webinar'
    )
  }
  return (
    <>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <AdminIconButton
                      action={() => {
                        router.push(
                          `/admin/webinars/edit_webinar/${item.id}?img_path=${item.picture}`
                        )
                      }}
                    >
                      <svg
